...
const createCart = async function (req, res) {
    ...
    try {
        ...
        let findCartOfUser = await cartModel.findOne({ userId: userId, isDeleted: false })
        if (productId && (!cartId)) {
        ...
        if (cartId&&findCartOfUser) {
            if (findCartOfUser._id.toString() != cartId)
...
const updateCart = async function (req, res) {
    ...
    let checkProductExist = await productModel.findOne({ _id: productId, isDeleted: false })
